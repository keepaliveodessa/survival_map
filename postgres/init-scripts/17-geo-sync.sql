...
--   Гребной канал       — OSM Nominatim (way 35028168, полигон; алиасы
--                         Гребний/Грибной канал — вторая форма и опечатка канала)
--   Орион               — Nominatim way 209742186 (ресторан на Житомирской,
--                         ориентир «Орион блокпост» у Заставы II; не промзона)
--   Окружная дорога     — Nominatim way 278896868 (Окружна вулиця, Нерубайское —
--                         вдоль объездной, там ОККО: «Окружная. ОККО»)
...
--   Великодолинское (Акаржа/Аккаржа/Акарж), Аркадийский (Аркадиевский/
--   Аркадьевский — частая опечатка канала, роняла пины ЖК «Морське»),
--   2 застава (застава 2 — реверс порядка слов давал stem_reorder 0.78 < 0.80),
--   Новая Долина (М долина, Новой долины — род. падеж направления),
--   Прилиманское (При лиманское — разрыв предлога в токенах).
...
BEGIN
    ...
    FOR r IN
        ...
        FROM (VALUES ('БКМ'), ('улица Бабеля'), ('улица Ждахи'),
                     ...
                     ('Великодолинское'), ('Балтская дорога'),
                     ('Гребной канал'), ('Орион'), ('Окружная дорога')) AS t(canon)
...
--    ВАЖНО: PERFORM — синтаксис plpgsql, на верхнем уровне .sql-файла (psql)
--    даёт «syntax error at or near PERFORM» и валит всю инициализацию
--    (postgres exit 3). Обёртка в DO-блок обязательна.
-- -----------------------------------------------------------------------------
DO $$
BEGIN
...
--   SELECT id, names FROM geo
--    WHERE names && ARRAY['застава 2','аркадиевский','грибной канал',
--                         'орион','окружная','при лиманское','новой долины']::text[];
...
--   «Застава 2 только выставили»              → 2 застава (было stem_reorder 0.78 < 0.80)
--   «Гребной/Грибной канал блокпост»          → Гребной канал (было пусто)
--   «Орион блокпост на остановке»             → Орион (было пусто)
--   «Окружная. ОККО»                          → Окружная дорога (было пусто)
--   «Аркадиевский переулок»                   → Аркадийский (было 0.87/фоллбек «строй»)
--   «за При Лиманским … Новой долины»         → Прилиманское / Новая Долина (было 0.78/пусто)
--

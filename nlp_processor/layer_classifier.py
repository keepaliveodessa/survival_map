...
class LayerClassifier:
    ...
    def classify(self, lemmas: List[Lemma], raw_text: str = None) -> str:
        ...
        if lemmas:
            ...
            # pymorphy лемматизирует опечатку неустойчиво: «блопост»→«блопоста»,
            # «блопостом»→«блопост» — проверяем обе формы + префикс надёжнее:
            # любая лемма на «блопост»/«блокпост» (блопоста, блокпостов…).
            if ('блокпост' in token_lemmas or 'бп' in token_lemmas
                    or 'блопост' in token_lemmas or 'блопоста' in token_lemmas
                    or any(l.startswith(('блопост', 'блокпост')) for l in token_lemmas)):
                result = 'traffic'

...
@pytest.mark.parametrize("text,expected", [
    ...
    ("ставят блопост, между Толбухина и третьей станцией", "traffic"),  # опечатка, events_export5 id 10
    ("блопост из мусоров и тцк во дворах", "traffic"),      # опечатка + HARD RULE 1 раньше standard-priority
    # Ложные леммы pymorphy (events_export8): род. падеж сленга ≈ фамилия,
    # словарь хранит обе взаимные леммы («мусоров»↔«мусорова», «меньов»↔«меньова»)
    ("На бугоевской не доезжая Дальницкая куча тцк и мусоров,осторожно", "cops"),
    ("Пятая Крыжановка две машины меньов в сторону Южного", "cops"),
    ("куча тцк и мусоров возле школы", "cops"),

...
DEFAULT_LAYER_KEYWORDS: dict[str, tuple] = {
    'bus': (
        ...
        'буса', 'бусик',
        # Тип события из заголовков шаблонных пинов: «Черный/Белый/Серый
        # транспорт» — перемещение транспорта ТЦК → слой bus (поправка
        # от владельца проекта; экспорт 4, пины 560312/560321 и др.).
        'транспорт',
    ),
    'cops': (
        'коп',
    ...
    'traffic': (
        ...
        'светофор',
        'блокпост',
        # Опечатка из прод-экспорта (events_export5 id 10): «ставят блопост,
        # между Толбухина и третьей станцией» → слой падал в pig.
        'блопост',
        'пост',
        'бп',
        'б/п'
    ),
    # Словарь pig проверяется ПОСЛЕДНИМ (после bus/cops/traffic) — слова
    # типов событий из заголовков шаблонных пинов (📍 … 🏠), не попавшие в
    # другие слои: «Тцк (пешие)» — пешие сотрудники ТЦК. Без совпадения в
    # ЛЮБОМ словаре шаблонный пин падает в pig-fallback (HARD RULE 2 в
    # layer_classifier) — словарь лишь делает выбор явным.
    'pig': (
        'тцк',
    ),
}
...
class GeoConfig:
    ...
    # Адресный fast-path для структурированных пинов (N4b): 📍 … 🏠 Адрес: …
    # 📝 Описание: … Перед матчем всего текста адрес парсится, самый специфичный
    # сегмент (с типом улицы) матчится прицельно через match_phrase; без
    # уверенного матча — fallback на общий путь. Kill-switch: False → все
    # сообщения идут по общему пути (старое поведение, байт-в-байт).
    structured_fastpath: bool = True
